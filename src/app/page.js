    ...
    <header className="modern-header">
      <div className="modern-header-container">
        <div className="header-content">
          ...
          <p>Transform Online Images into High-Quality Prints</p>
        </div>
    ...
    <div className="legend-items">
      {[
        ...
        <div key={index} className="legend-item">
          <span className="color-swatch" style={{ backgroundColor: getGradeColor(item.dpi) }}></span>
...
const PosterPrintChecker = () => {
  ...
  const handleCustomSizeSubmit = (width, height, unit) => {
    ...
    analyzeImage(image.width, image.height, [...COMMON_SIZES, ...customSizes, newSize]);
  };

  const analyzeImage = (width, height, sizes = [...COMMON_SIZES, ...customSizes]) => {
                ...
                <div className="size-selector">
                  ...
                  <div className="size-grid">
                    {analysis && analysis.map((result, index) => {
                      const sizeInfo = [...COMMON_SIZES, ...customSizes].find(size => size.name === result.size);
                      return (
                        <button 
                          key={index} 
                          className={`size-option ${selectedSize === result.size ? 'selected' : ''}`}
                          style={{ backgroundColor: getGradeColor(result.dpi) }}
                          onClick={() => {
                  ...
                  <CustomSizeInput onCustomSizeSubmit={handleCustomSizeSubmit} />
                </div>
                <GradeLegend />
                {selectedSize && (
                  <div className="size-details">
                    ...
                    <p><strong>DPI:</strong> {analysis.find(a => a.size === selectedSize)?.dpi}</p>
